...
ggplot(dataIC50, aes(x=Scenario, y=log10Value, fill=Scenario)) + geom_boxplot() + facet_wrap(~Scenario, scale="free") + theme(text = element_text(size=15), legend.text = element_text(size=14), axis.title = element_text(size=17), axis.title.x=element_blank(), axis.text.x=element_blank()) + ylab("Log10 IC50")

dataIC80 <- melt(combinedIC80)
colnames(dataIC80) <- c("number", "Scenario", "log10Value")
ggplot(dataIC80, aes(x=Scenario, y=log10Value, fill=Scenario)) + geom_boxplot() + facet_wrap(~Scenario, scale="free") + theme(text = element_text(size=15), legend.text = element_text(size=14), axis.title = element_text(size=17), axis.title.x=element_blank(), axis.text.x=element_blank()) + ylab("Log10 IC80")
```

## Histograms
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=7}

ggplot(dataIC50, aes(x=log10Value, y=..count../sum(..count..), fill=Scenario)) + geom_histogram() + facet_wrap(~Scenario) + ylab("Density")+ theme(text = element_text(size=15), legend.text = element_text(size=14), axis.title = element_text(size=17)) + xlab("Log10 IC50")

ggplot(dataIC80, aes(x=log10Value, y=..count../sum(..count..), fill=Scenario)) + geom_histogram() + facet_wrap(~Scenario) + ylab("Density")+ theme(text = element_text(size=15), legend.text = element_text(size=14), axis.title = element_text(size=17)) + xlab("Log10 IC80")
```